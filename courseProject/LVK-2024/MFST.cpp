...
namespace MFST
{
	...
	bool Mfst::start(const Log::LOG& log)
	{
		...
		case LENTA_END:         MFST_TRACE4(log, "------>LENTA_END")
			* log.stream << "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~" << std::endl;
			sprintf_s(buf, MFST_DIAGN_MAXSIZE, "%d:total lines %d, syntactic analysis completed successfully ", 0, lenta_size);
			*log.stream << std::setw(4) << std::left << 0 << ":total lines " << lenta_size << ", syntactic analysis completed successfully " << std::endl;
			rc = true;
			break;
