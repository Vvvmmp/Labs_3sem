...
namespace Polish
{
	...
	bool __cdecl setPolishNotation(IT::IdTable& idtable, Log::LOG& log, int lextable_pos, ltvec& v)
	{
		...
		for (unsigned i = 0; i < v.size(); i++)
		{
			...
			if (v[i].lexema == LEX_LEFTHESIS || v[i].lexema == LEX_RIGHTTHESIS ||
				v[i].lexema == LEX_PLUS || v[i].lexema == LEX_MINUS ||
				v[i].lexema == LEX_STAR || v[i].lexema == LEX_DIRSLASH ||
				v[i].lexema == LEX_LEFT || v[i].lexema == LEX_RIGHT)
			{
		...
		if (!result.empty()) {
			int currentLine = result[0].sn; 
			string lineOutput = "";         

			if (!mark) {
				cout << "Польская запись (с номерами строк): " << endl;
				mark = true;
			}

			for (auto entry : result) {
				lineOutput += entry.lexema;   
			}

			cout << "Строка " << currentLine << " : " << lineOutput << endl;
		}
