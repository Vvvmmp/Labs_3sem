...
namespace Semantic
{
	bool Semantic::semanticsCheck(Lexer::LEX& tables, Log::LOG& log)
	{
		...
		for (int i = 0; i < tables.lextable.size; i++)
		{
				...
				if (i > 0 && tables.lextable.table[i - 1].idxTI != NULLIDX_TI)
				{
					...
					for (int k = i + 1; tables.lextable.table[k].lexema != LEX_SEPARATOR; k++)
					{
						...
						if (lefttype == IT::IDDATATYPE::STR)
						{
							...
							//char prevl = tables.lextable.table[k - 1].lexema;
							//char nextl = tables.lextable.table[k + 1].lexema;
							//if (l == LEX_MINUS && prevl == LEX_LITERAL && nextl == LEX_LITERAL) {
							//	if (prevl > nextl) {
			...
			case LEX_MORE:	case LEX_LESS: case LEX_EQUALS:   case LEX_NOTEQUALS: case LEX_MOREANDEQUAL: case LEX_LESSANDEQUAL:
			{
				bool flag = true;
