...
namespace IT
{
	...
	void writeIdTable(std::ostream* stream, IT::IdTable& idtable)
	{
		...
		*stream << "|  ¹  |Ëèíèÿ â LT |   ID        Òèï    |      Íàçâàíèå     |  Çíà÷åíèå(àðãóìåíòû)    " << std::endl;
		for (int i = 0; i < idtable.size; i++)
		{
			...
			case IT::IDTYPE::L:
				strcat(type, "   literal");
				break;
			case IT::IDTYPE::S: strcat(type, "  PIE LIBf"); break;
			default:
				strcat(type, "UNDEFINED ");
				break;
			}
